...
bool    ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    g_GlVersion = static_cast<GLuint>(major * 100 + minor * 10);
#else
...
static void ImGui_ImplOpenGL3_SetupRenderState(ImDrawData* draw_data, const int fb_width, const int fb_height, GLuint vertex_array_object)
{
    ...
    auto clip_origin_lower_left = true;
...
void    ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
    ...
    const auto last_enable_stencil_test = glIsEnabled(GL_STENCIL_TEST);
    const auto last_enable_scissor_test = glIsEnabled(GL_SCISSOR_TEST);
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_PRIMITIVE_RESTART
    const auto last_enable_primitive_restart = (g_GlVersion >= 310) ? glIsEnabled(GL_PRIMITIVE_RESTART) : GL_FALSE;
#endif
    ...
    const auto clip_off = draw_data->DisplayPos;         // (0,0) unless using multi-viewports
    const auto clip_scale = draw_data->FramebufferScale; // (1,1) unless using retina display which are often (2,2)
    ...
    for (auto n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, static_cast<GLsizeiptr>(cmd_list->VtxBuffer.Size) * static_cast<int>(sizeof(ImDrawVert)), static_cast<const GLvoid*>(cmd_list->VtxBuffer.Data), GL_STREAM_DRAW);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, static_cast<GLsizeiptr>(cmd_list->IdxBuffer.Size) * static_cast<int>(sizeof(ImDrawIdx)), static_cast<const GLvoid*>(cmd_list->IdxBuffer.Data), GL_STREAM_DRAW);
        ...
        for (auto cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
                {
                    ...
                    glScissor(static_cast<int>(clip_rect.x), static_cast<int>(fb_height - clip_rect.w), static_cast<int>(clip_rect.z - clip_rect.x), static_cast<int>(clip_rect.w - clip_rect.y));

                    // Bind texture, Draw
                    glBindTexture(GL_TEXTURE_2D, static_cast<GLuint>(reinterpret_cast<intptr_t>(pcmd->TextureId)));
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_VTX_OFFSET
                    if (g_GlVersion >= 320)
                        glDrawElementsBaseVertex(GL_TRIANGLES, static_cast<GLsizei>(pcmd->ElemCount), sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, reinterpret_cast<void*>(static_cast<intptr_t>(pcmd->IdxOffset * sizeof(ImDrawIdx))), static_cast<GLint>(pcmd->VtxOffset));
                    else
#endif
                        glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(pcmd->ElemCount), sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, reinterpret_cast<void*>(static_cast<intptr_t>(pcmd->IdxOffset * sizeof(ImDrawIdx))));
                }
            }
        }
    }
...
bool    ImGui_ImplOpenGL3_CreateDeviceObjects()
{
    ...
    g_AttribLocationVtxPos = static_cast<GLuint>(glGetAttribLocation(g_ShaderHandle, "Position"));
    g_AttribLocationVtxUV = static_cast<GLuint>(glGetAttribLocation(g_ShaderHandle, "UV"));
    g_AttribLocationVtxColor = static_cast<GLuint>(glGetAttribLocation(g_ShaderHandle, "Color"));
